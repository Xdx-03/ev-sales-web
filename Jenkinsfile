...
pipeline {
    ...
    parameters {
        ...
        booleanParam(
            name: 'RUN_LIVE_TESTS',
            defaultValue: false,
            description: '显式开启真实 UI 回归；关闭时执行静态检查、场景收集和合成框架检查'
        )
    }
                ...
                script {
                    if (isUnix()) {
                        sh '''
                            ...
                            .venv/bin/python -m compileall -q ev_web tests framework_checks scripts run_web_tests.py
        ...
        always {
            script {
                ...
                if (liveTestsStarted) {
                    ...
                    archiveArtifacts(
                        artifacts: 'allure-results/**,artifacts/**,reports/junit.xml',
